...
def main():
    ...
    save_root = "results/local_stationary_flow"
    ...
    boundary[L-1:L+1] = True; boundary[:, L-1:L+1] = True
    ...
    Ftrue = np.zeros((Nf*Nf, Nf*Nf))
    F_trans = np.zeros((4, Nf*Nf, Nf*Nf))
    for ii in range(4):
        F_trans[ii] = translation_matrix4(Nf,Nf,ii)
    for ii, bs in enumerate(B):
        for jj in range(L):
            for kk, b in enumerate(bs):
                base = ii*bL2+jj*bL
                Ftrue[base+kk*L:base+(kk+1)*L] = F_trans[b,base+kk*L:base+(kk+1)*L]
