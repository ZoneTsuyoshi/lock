...
class SpatiallyUniformLOCK(object) :
    ...
    def forward(self):
        ...
        for t in range(T) :
            ...
            if self.xp.any(self.xp.isnan(self.y[t])) :
                ...
                if (not self.advance_mode) and t-1>self.update_interval and (t-1)%self.update_interval==0:                    self._update_transition_matrix(t)
    ...
    def _update_transition_matrix(self, t):
        ...
        Xi = self.xp.zeros((self.n_dim_obs*self.update_interval, self.Amax))
        for a in range(self.Amax):
            y_repeat = self.xp.repeat(self.y[t-self.update_interval:t], self.n_dim_obs, axis=0)
            A_tile = self.xp.tile(self.xp.where(self.A==a+1, True, False), (self.update_interval, 1))
            y_repeat[~A_tile] = 0
