...
class Scheme(object):
    ...
    def noise_added_simulation(self, initial_x, sys_sd, obs_sd):
        ...
        for s in range(self.timestep-1):
            current_x = self._forward(current_x, s*self.dt) + self.xp.random.normal(0, sys_sd, size=len(initial_x))
...
class ModifiedEulerScheme(Scheme):
    ...
    def _forward(self, x, t):
        k1 = self.system.f(x, t)
        k2 = self.system.f(x + k1*self.dt, t + self.dt)
...
class RungeKuttaScheme(Scheme):
    ...
    def _forward(self, x, t):
        k1 = self.system.f(x, t)
        k2 = self.system.f(x + k1*self.dt/2, t + self.dt/2)
        k3 = self.system.f(x + k2*self.dt/2, t + self.dt/2)
        k4 = self.system.f(x + k3*self.dt, t + self.dt)
